...
class Pager
{
    ...
    public function __construct($data = array())
    {
		...
		$this->limit     = "limit ". ($this->now_page-1)*$this->list_rows.",$this->list_rows";   
    ...
    protected function _get_link($page,$text)
    {
        switch ($this->method) {
            case 'ajax':
				...
				$url_para = $this->_get_url($page);
				return "<a onclick='" . $this->ajax_func_name . "(\"" . $page . "\"".$parameter.")' href='javascript:showpage(\"$url_para\")'>" . $text . "</a>" . "\n";
            break;
    ...
    { 
		switch ($this->method) {
			case 'ajax':
				...
				$url_para = $this->url.$this->page_name.'=';
				return "<input type='text' value='".$this->now_page."'/><input type='button' value='GO' onclick='javascript:showpage(\"$url_para\" + this.previousSibling.value)'/>";
			break;
			...
			default:
				return "<table><form action=$this->url method='get'>跳转<input type='text' name='$this->page_name'/><input type='submit' value='GO'/></form></table>";
			break;
		}
    }
